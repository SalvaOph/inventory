    ...
    <form action="{{ isset($inventory) ? route('inventories.update', $inventory->id) : route('inventories.store') }}" method="POST">
        ...
        <div class="form-group">
            <strong>Product:</strong>
                <input type="text" name="product_name" value="{{ isset($inventory) ? $inventory->product->name : '' }}" class="form-control" disabled="disabled">
                <input type="hidden" name="product_id" value="{{ isset($inventory) ? $inventory->product->id : '' }}">
            </select><br>
        </div>
        <div class="form-group">
            <strong>Stock:</strong>
